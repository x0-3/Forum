-- --------------------------------------------------------
-- Host:                         127.0.0.1
-- Server version:               8.0.30 - MySQL Community Server - GPL
-- Server OS:                    Win64
-- HeidiSQL Version:             12.1.0.6537
-- --------------------------------------------------------

/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET NAMES utf8 */;
/*!50503 SET NAMES utf8mb4 */;
/*!40103 SET @OLD_TIME_ZONE=@@TIME_ZONE */;
/*!40103 SET TIME_ZONE='+00:00' */;
/*!40014 SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0 */;
/*!40101 SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO' */;
/*!40111 SET @OLD_SQL_NOTES=@@SQL_NOTES, SQL_NOTES=0 */;


-- Dumping database structure for forum
CREATE DATABASE IF NOT EXISTS `forum` /*!40100 DEFAULT CHARACTER SET latin1 */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `forum`;

-- Dumping structure for table forum.category
CREATE TABLE IF NOT EXISTS `category` (
  `id_category` int NOT NULL AUTO_INCREMENT,
  `nameCategory` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci DEFAULT NULL,
  PRIMARY KEY (`id_category`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=latin1;

-- Dumping data for table forum.category: ~4 rows (approximately)
INSERT INTO `category` (`id_category`, `nameCategory`) VALUES
	(1, 'Album review'),
	(2, 'New Releases'),
	(3, 'Soul & Funk'),
	(4, 'R & B');

-- Dumping structure for table forum.like
CREATE TABLE IF NOT EXISTS `like` (
  `user_id` int NOT NULL,
  `topic_id` int NOT NULL,
  KEY `FK_like_topic` (`topic_id`),
  KEY `FK_like_user` (`user_id`),
  CONSTRAINT `FK_like_topic` FOREIGN KEY (`topic_id`) REFERENCES `topic` (`id_topic`),
  CONSTRAINT `FK_like_user` FOREIGN KEY (`user_id`) REFERENCES `user` (`id_user`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Dumping data for table forum.like: ~3 rows (approximately)
INSERT INTO `like` (`user_id`, `topic_id`) VALUES
	(20, 12),
	(21, 13),
	(20, 13),
	(22, 13),
	(22, 12);

-- Dumping structure for table forum.message
CREATE TABLE IF NOT EXISTS `message` (
  `id_message` int NOT NULL AUTO_INCREMENT,
  `text` text CHARACTER SET latin1 COLLATE latin1_swedish_ci,
  `messCreatedAt` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `user_id` int NOT NULL,
  `topic_id` int NOT NULL,
  PRIMARY KEY (`id_message`),
  KEY `FK__user` (`user_id`),
  KEY `FK_message_topic` (`topic_id`),
  CONSTRAINT `FK__user` FOREIGN KEY (`user_id`) REFERENCES `user` (`id_user`),
  CONSTRAINT `FK_message_topic` FOREIGN KEY (`topic_id`) REFERENCES `topic` (`id_topic`)
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=latin1;

-- Dumping data for table forum.message: ~9 rows (approximately)
INSERT INTO `message` (`id_message`, `text`, `messCreatedAt`, `user_id`, `topic_id`) VALUES
	(1, 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi congue rhoncus nibh sed pretium. Sed eleifend eleifend mi, quis vestibulum sapien molestie sed. Nulla ac porta dui, semper gravida nisi. Cras varius ultrices pharetra. Curabitur lorem ligula, mattis euismod nisi eget, ultricies maximus libero. Nullam hendrerit et leo nec lacinia.', '2023-03-21 19:12:07', 1, 2),
	(2, 'Curabitur justo magna, maximus id ultrices vitae, egestas in erat. Vivamus sed dapibus sem. Mauris cursus risus sed purus vehicula, vel placerat massa convallis. Ut tristique diam nec sapien tempus, non consectetur elit ultricies. Fusce nec gravida nisi. Mauris vel quam vel sapien tincidunt mollis', '2023-03-21 19:14:19', 2, 1),
	(3, 'test', '2023-03-24 11:13:12', 1, 2),
	(4, 'dxfcgv', '2023-03-29 18:15:17', 1, 3),
	(5, 'tdxcfgvbhj', '2023-03-30 22:06:04', 20, 3),
	(6, 'dfghbj', '2023-03-30 22:06:18', 20, 3),
	(7, 'wdxfcvg', '2023-03-30 22:06:43', 20, 3),
	(8, 'dfghj', '2023-03-30 22:17:35', 20, 3),
	(9, 'hello', '2023-03-30 22:17:56', 20, 3),
	(10, 'hello', '2023-03-30 22:18:21', 20, 3),
	(11, 'dfghjk', '2023-03-31 09:05:07', 20, 12);

-- Dumping structure for table forum.topic
CREATE TABLE IF NOT EXISTS `topic` (
  `id_topic` int NOT NULL AUTO_INCREMENT,
  `title` text CHARACTER SET latin1 COLLATE latin1_swedish_ci,
  `topicCreatedAt` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `lockTopic` tinyint NOT NULL DEFAULT '0',
  `like` int NOT NULL,
  `category_id` int NOT NULL,
  `user_id` int NOT NULL,
  PRIMARY KEY (`id_topic`) USING BTREE,
  KEY `FK__category` (`category_id`),
  KEY `FK_topic_user` (`user_id`),
  CONSTRAINT `FK__category` FOREIGN KEY (`category_id`) REFERENCES `category` (`id_category`),
  CONSTRAINT `FK_topic_user` FOREIGN KEY (`user_id`) REFERENCES `user` (`id_user`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=latin1;

-- Dumping data for table forum.topic: ~13 rows (approximately)
INSERT INTO `topic` (`id_topic`, `title`, `topicCreatedAt`, `lockTopic`, `like`, `category_id`, `user_id`) VALUES
	(1, 'What (Soul/R&B) Are You Listening To Right Now?', '2023-03-21 19:02:24', 0, 0, 3, 1),
	(2, 'Sunday R&B Recommendation Thread - get/give recommendations here!', '2023-02-21 19:05:28', 0, 0, 4, 2),
	(3, 'test', '2023-03-24 11:39:44', 0, 0, 4, 1),
	(4, 'fcgvbh', '2023-03-29 18:24:40', 0, 0, 2, 13),
	(5, 'fgvhbjn', '2023-03-30 22:23:55', 0, 0, 3, 20),
	(6, 'test', '2023-03-30 22:25:15', 0, 0, 3, 20),
	(7, 'dtfghb', '2023-03-30 22:26:19', 0, 0, 3, 20),
	(8, 'cfgvbhjn,', '2023-03-30 22:28:51', 0, 0, 3, 20),
	(9, 'cfgvbhjn,', '2023-03-30 22:30:23', 0, 0, 3, 20),
	(10, 'cfgvbhjn,', '2023-03-30 22:30:45', 0, 0, 3, 20),
	(11, 'xdcgfbhj', '2023-03-30 22:30:52', 0, 1, 3, 20),
	(12, 'xdcgfbhj', '2023-03-30 22:31:09', 0, 2, 3, 20),
	(13, 'cfgvbh', '2023-03-30 22:31:36', 0, 3, 3, 20);

-- Dumping structure for table forum.user
CREATE TABLE IF NOT EXISTS `user` (
  `id_user` int NOT NULL AUTO_INCREMENT,
  `pseudo` varchar(50) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL,
  `avatar` text CHARACTER SET latin1 COLLATE latin1_swedish_ci,
  `email` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL,
  `password` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL,
  `signUpDate` datetime DEFAULT CURRENT_TIMESTAMP,
  `role` json DEFAULT NULL,
  PRIMARY KEY (`id_user`)
) ENGINE=InnoDB AUTO_INCREMENT=23 DEFAULT CHARSET=latin1;

-- Dumping data for table forum.user: ~10 rows (approximately)
INSERT INTO `user` (`id_user`, `pseudo`, `avatar`, `email`, `password`, `signUpDate`, `role`) VALUES
	(1, 'Hichat', 'data:image/png;base64,', 'MichaelLLafrance@teleworm.us', 'ax9eJeeweel', '2023-03-21 18:57:17', '0'),
	(2, 'Nage1946', 'data:image/png;base64,', 'SarahEStewart@rhyta.com', 'TeChohS1h', '2023-02-21 18:58:39', '0'),
	(13, 'testtest1', 'public/upload/9b0b4a43ab10092a7e001e972f532c4c307847be31ac36581ee93c0d21d66a71', 'pseudo@pseudo.com', '$2y$10$e.cmIZZELsbsmL/5WNBpBeXBSrGyZ9HrRBq.cJrIBXFAzXvLuFNeS', '2023-03-29 19:34:49', NULL),
	(14, 'mevdy', 'public/upload/642480ccb22f1logOut.png', 'pseudo@pseudo.com', '$2y$10$ee/BUFLI7yCesDY6TAzlw.PM0deHttKzGGV8LytC34IQsQXBe3xfC', '2023-03-29 20:17:48', NULL),
	(15, 'test5', 'public/upload/6425395f689adlogOut.png', 'test@test.fr', '$2y$10$TYZ4ASdsHComxEB0Lgd0TOg5QYz2t9/3ZDT8Jb0onSp2rilDtkyRW', '2023-03-30 09:25:19', NULL),
	(16, 'test6', 'public/upload/64253e57bc10asearch.png', 'test@test.fr', '$2y$10$RmYOo74A64BSp2MUi9HLAOLntK2/A2tfJHKiyoACZ8YpU/My0GN82', '2023-03-30 09:46:31', NULL),
	(17, 'ydfhgvb', 'public/upload/64253ef55b7bcimage-removebg-preview.png', 'test@test', '$2y$10$Fc5QhZK3zHErepQ78nhVAevpjfcqVJdZ.Sav.bUtoYRHpMu6EG/qi', '2023-03-30 09:49:09', NULL),
	(18, 'ufjghbkj', 'public/upload/64253f3961359search.png', 'test@test', '$2y$10$l9KfUEi8NSmjyz6SfmbCG.qL.hCLtcWSgMwh9xtL1gszusp9tct0C', '2023-03-30 09:50:17', NULL),
	(19, 'chgvjhb', 'public/upload/64253f63aa773Screenshot 2023-03-29 105402.png', 'test@test.fr', '$2y$10$nEFAGdHCEl0aS7YiEyJuN.r8rb/47eMKvxmK9Rcf8/j16PHSfB1.K', '2023-03-30 09:50:59', NULL),
	(20, 'test8', 'public/upload/64254204a64eflogOut.png', 'test@test.fr', '$2y$10$8JyTvtCOEF0F1R4tmeUeUO6.6xSzkEowQvYFXSInyyNybJt2RbuSS', '2023-03-30 10:02:12', '0'),
	(21, 'azer', 'public/upload/6426c7546f44bsearch.png', 'azer@test.fr', '$2y$10$EGyQtuFtXGvwJXXl9Yx9rOArukQ6.BQlPAyGGhxIgalDyY5xPT4B.', '2023-03-31 13:43:16', '0'),
	(22, 'geralt', 'public/upload/6429a66488ec7download.png', 'geralt@test.fr', '$2y$10$9mxg8o0nsAttZtIHpEowYuyawTOVAXv4.hMWXqIIXQOJf517pYVtG', '2023-04-02 17:59:32', NULL);

/*!40103 SET TIME_ZONE=IFNULL(@OLD_TIME_ZONE, 'system') */;
/*!40101 SET SQL_MODE=IFNULL(@OLD_SQL_MODE, '') */;
/*!40014 SET FOREIGN_KEY_CHECKS=IFNULL(@OLD_FOREIGN_KEY_CHECKS, 1) */;
/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40111 SET SQL_NOTES=IFNULL(@OLD_SQL_NOTES, 1) */;
